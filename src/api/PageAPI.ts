...
export async function getPages(): Promise<PageModel[]> {
    const url: string = `http://localhost:8080/page`;
    return (await getPage(url)).result;
}


export async function getListPage(page: number): Promise<ResultInterface> {
    const url: string = `http://localhost:8080/page?sort=pageId,asc&size=8&page=${page}`;
    return getPage(url);
}

export async function deletePage(pageId: number) {
    const url: string = `http://localhost:8080/page/${pageId}`;
